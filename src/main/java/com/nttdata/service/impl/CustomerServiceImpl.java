...
public class CustomerServiceImpl implements CustomerService {
    ...
    public Mono<Customer> create(Customer c) {
        if (!StringUtils.hasText(c.getDocumentNumber())) {
            return Mono.error(new ResponseStatusException(HttpStatus.BAD_REQUEST, "documentNumber is required"));
        }
        if (c.getType() == null) {
        ...
        return repo.existsByDocumentNumberAndActiveIsTrue(c.getDocumentNumber())
                .flatMap(exists -> exists
                        ? Mono.error(new ResponseStatusException(HttpStatus.CONFLICT, "Document already exists"))
                        : repo.save(c))
    ...
    public Flux<Customer> findAll(CustomerType type, CustomerSegment segment) {
        return Optional.ofNullable(type)
                .map(t -> Optional.ofNullable(segment)
    ...
    public Mono<Customer> update(String id, Customer c) {
        return repo.findById(id)
                ...
                .flatMap(db -> {
                    ...
                    if (msg != null) {
                        return Mono.error(new ResponseStatusException(HttpStatus.UNPROCESSABLE_ENTITY, msg));
                    }

                    if (Boolean.TRUE.equals(db.getActive())) {
                        return repo.existsByDocumentNumberAndActiveIsTrueAndIdNot(db.getDocumentNumber(), id)
                                .flatMap(exists -> exists
                                        ? Mono.error(new ResponseStatusException(HttpStatus.CONFLICT, "Document already exists"))
                                        : repo.save(db))
                                .onErrorMap(DuplicateKeyException.class,
    ...
    public Mono<Customer> findActiveByDocumentNumber(String documentNumber) {
        ...
        return repo.findByDocumentNumberAndActiveIsTrue(documentNumber)
                ...
                .flatMap(list -> {
                    ...
                    if (list.size() > 1) {
                        return Mono.error(new ResponseStatusException(
                                HttpStatus.CONFLICT, "More than one active customer with this documentNumber"));
                    }
